---
title: "Mapping US Census Data with R Workshop"
author:
...
To get the specific variable names can also be tricky. One place to look on the Census website is called "Table Shells." We can find the [detailed table shell for ACS 2023](https://www2.census.gov/programs-surveys/acs/summary_file/2023/table-based-SF/documentation/ACS2023_Table_Shells.xlsx) and then navigate to the B25003 table to find the specific variables to call in R.
...
**Choropleth map showing all tracts by level of income, with an overlay bubble map of population**
...
Finally, let's spend some time practicing making maps.
...
prac_data_clean <- prac_data |> 
  # Remove some columns, relabel others
  select(GEOID,
         NAME,
         -variable,
         gini_value = estimate,
         geometry) |> 
  # Create a State and county column
  separate(col = "NAME",
           into = c("county", "state"),
           sep = ", ",
           remove = TRUE)

# Going to use {tmap} again since it has a facet function for multiple maps

prac_tmap <- tmap::tm_shape(shp = prac_data_clean) +
...
### Bonus practice: Bring in external data

*We may not have time for this one, and I didn't include a solution to review, but I'm going to leave the prompt if anyone wants to explore it afterwards.*
...
[Kyle Walker's census data book](https://walker-data.com/census-r/)
...
[UVA Spatial Technology libguide](https://guides.lib.virginia.edu/gis)

Email:

If you have any questions, I'm happy to follow up with a one-on-one consult as well or you can send a request to the UVA Research Data Management email <dmconsult@virginia.edu>.

Spatial questions (e.g., map projections, spatial techniques, ArcGIS) can be answered by the UVA Geographic Information Science (GIS) support group <uvagis@virginia.edu>. They also teach spatial workshops through each semester.
